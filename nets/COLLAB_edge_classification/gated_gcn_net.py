...
class GatedGCNNet(nn.Module):
    ...
    def loss(self, pos_out, neg_out):
        ...
        # alpha = 100
        # beta = 0.1
        # false_positive_cost = alpha * torch.mean(neg_out)  # Cost for false positives
        # false_negative_cost = beta * torch.mean(1 - pos_out)  # Cost for false negatives
    
        # cost_sensitive_loss = pos_loss + neg_loss + false_positive_cost + false_negative_cost
    
        # return cost_sensitive_loss
        
        
    
    # def cost_sensitive_loss(self, output, target, alpha=1.0, beta=1.0):
    #     loss = nn.CrossEntropyLoss()(output, target)  # Regular Cross-Entropy Loss
    #     false_positive_loss = alpha * torch.sum(output[:, 1] * (1 - target))  # Cost for false positives
    #     false_negative_loss = beta * torch.sum((1 - output[:, 1]) * target)  # Cost for false negatives
    #     total_loss = loss + false_positive_loss + false_negative_loss
    #     return total_loss
