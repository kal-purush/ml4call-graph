...
to_do_list =['lodash', 'underscore', 'eslint-plugin-import', 'mocha', 'angular', 'shelljs', 'js-yaml', 'commander.js', 'bluebird', 'express', 'async', 'jquery', 
                'handlebars.js', 'bootstrap', 'immutable-js', 'package', 'q', 'formula-parser', 'request', 'less.js', 
             'eslint-plugin-react', 'axios', 'react']
app_list =  ['lodash', 'underscore', 'eslint-plugin-import', 'mocha', 'angular', 'shelljs', 'js-yaml', 'commander.js', 'bluebird', 'express', 'async', 'jquery', 
             'handlebars.js', 'bootstrap', 'immutable-js', 'package', 'q', 'formula-parser', 'request', 'less.js', 
             'eslint-plugin-react', 'axios', 'react']

# to_do_list = ['eslint-plugin-import','bootstrap', 'immutable-js', 'less.js']
app_list = ['mysql', 'joi','node-jsonwebtoken', 'create-react-app', 'react-router', 'UglifyJS']
# app_list = ['chai', 'autoprefixer', 'winston', 'node-semver', 'html-webpack-plugin', 'qs', 'chokidar', 'postcss', 'postcss-loader', 'ejs', 'morgan', 'url-loader']
...
for i in range(len(app_list)):
    try:
        ...
        # EPOC_NUMBER = 78
        print(APPLICATION_NAME, " =====> ", EPOC_NUMBER)
        ...
        # print(touple_list)
        output_file = open("missing_edges_log/"+APPLICATION_NAME+"_missing_edge_log_"+MODEL_NAME+".txt","w+")
        cutoff_thrshld = 0.9
        node_df_org = pd.read_csv("../prune_new/"+APPLICATION_NAME+"_node.csv")
        ...
        for i in range(len(touple_list)):
            try:
                ...
                # print(file_name, name, start_line, start_column, SOURCE, TARGET)
                ...
                if first_index == TARGET:
                    count+=1
                print("Source ===>",SOURCE, " FIRST ID ==> ", first_index, " Val ==> ", result_df.iloc[0]['val'])
            except:
                pass
        
        output_file.write("RANK 1 number of edge "+str(count)+" ===>  "+str(count/len(test_pred))+"\n")
        output_file.write("\n\n")    
        output_file.flush()
    except:
        raise
