...
class DatasetWithFunctionAndChildNode(Dataset):
    def __init__(self, name, WITHOUT_NAME_FEATURE=False, TRAIN_ONLY_FUNCTION_EDGE=False):
        start = time.time()
        print("[I] Loading dataset %s..." % (name), WITHOUT_NAME_FEATURE)
        self.name = name
        # self.dataset = DglLinkPropPredDataset(name='ogbl-collab')
        self.graph, train_pos_df, val_pos_df, test_pos_df, test_neg_df, val_neg_df= self.create_graph(without_name_feature=WITHOUT_NAME_FEATURE, TRAIN_ONLY_FUNCTION_EDGE=TRAIN_ONLY_FUNCTION_EDGE)  # single DGL graph
        print(self.graph)
        # print("Mian ==============>")
        # print(val_neg_df)
        import torch
    ...
    def create_graph(self, add_arg_len=True, add_param_len=True, add_name= True, without_name_feature=False, TRAIN_ONLY_FUNCTION_EDGE=False):
        import pandas as pd
        import torch.nn.functional as F
        import torch

        APPLICATION_NAME = 'vue'
        node_file_path = APPLICATION_NAME+"_nodes_with_only_childern_node.csv"
        edge_file_path = APPLICATION_NAME+"_edges_with_only_childern_node.csv"
        function_edge_file_path = APPLICATION_NAME+"_function_edges.csv"
        if APPLICATION_NAME == 'vue':
            node_file_path = 'only_function_nodes_vue.csv'
            edge_file_path = 'only_function_edge_vue.csv'
            function_edge_file_path = 'only_function_actual_edge_vue.csv'

        
        ROOT_PATH = "/Users/masudulhasanmasudbhuiyan/Music/test_ast_javascript/"

        df = pd.read_csv(ROOT_PATH+node_file_path)
        # print(df)
        nodes_data = df.drop(["start_line","start_column","end_line",  "end_column" ,"file_name"], axis=1)
        # nodes_data
        if without_name_feature:
            dummy_nodes, dummy_edge = self.add_dummy_nodes(nodes_data, global_id=len(nodes_data))
            nodes_data = pd.concat([nodes_data,dummy_nodes]).reset_index().drop(["index"], axis=1)
        # print(nodes_data)

        df = pd.read_csv(ROOT_PATH+edge_file_path, header=None)
        if without_name_feature:
        ...
        test_edges= pd.read_csv(ROOT_PATH+function_edge_file_path, header=None)
        ...
        # # val_pos_df = val_func_df
        # test_pos_df = test_func_df

        if TRAIN_ONLY_FUNCTION_EDGE:
            train_pos_df = train_func_df
            val_pos_df = val_func_df
            test_pos_df = test_func_df
        else:
    ...
    def add_dummy_nodes(self, nodes_data, global_id):
