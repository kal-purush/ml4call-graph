...
class NewDataset(Dataset):
    ...
    def get_function_edge(self, PATH):
        ...
        #     leftout.to_csv("leftout_"+self.name+".csv", index=False)
        #     return train_df
        
        df = pd.read_csv(PATH)
        # leftout_df = pd.read_csv("leftout_"+self.name+".csv")
        # function_edges = df.merge(leftout_df, indicator=True, how='outer').query('_merge=="left_only"').drop('_merge', axis=1)
        # return function_edges
        return df
    
    def remove_test_data(self, dynamic_edges):
        df = pd.read_csv("true_value_"+self.name+".csv")
    ...
    def get_test_edge(self, ROOT_PATH, name):
        ...
        dynamic_edges = pd.read_csv("dynamic_edges/prune_dynamic_edges_"+name+".csv")
        dynamic_edges = dynamic_edges.drop_duplicates(keep='first').reset_index(drop=True)

        if self.name == 'lodash':
            dynamic_edges = self.remove_test_data(dynamic_edges)

        dynamic_train_edge = dynamic_edges[:int(.8*len(dynamic_edges))]
        dynamic_test_edge = dynamic_edges[int(.8*len(dynamic_edges)):]

        # dynamic_train_edge = dynamic_edges

        # df = pd.concat([dynamic_train_edge, train_func_df])
        df = pd.concat([dynamic_train_edge, train_func_df, test_func_df])
        df = df.drop_duplicates(keep='first')
        
        # test_df =  pd.concat([dynamic_test_edge, test_func_df])
        test_df = dynamic_test_edge
