...
def get_test_edges(PATH, EPOC_NUMBER, model, graph, params, DATASET_NAME, MODEL_NAME, APPLICATION_NAME):
    df = pd.read_csv("results/NEW_DATASET--"+APPLICATION_NAME+"/GatedGCN/POS_PRED/pred_score_with_0_pos.csv", header=None)
...
def get_candidates(PATH, EPOC_NUMBER, model, graph, params, DATASET_NAME, MODEL_NAME, APPLICATION_NAME, SOURCE_ID, TARGET):
    ...
    for i in range(len(node_df)):
        try:
            ...
            test_edges = get_test_data(file_name, src_id, APPLICATION_NAME)
...
MODEL_NAME = 'GatedGCN'
to_do_list = ['lodash', 'formula-parser', 'js-yaml']
app_list = ['lodash', 'formula-parser', 'js-yaml']

# to_do_list = [ 'lodash', 'formula-parser', 'js-yaml', 'winston', 'ws', 'qs', 'node-fs-extra', 'UglifyJS']
# app_list = ['lodash', 'formula-parser', 'js-yaml', 'winston', 'ws', 'qs', 'node-fs-extra', 'UglifyJS'] 
app_list = ['mathjs', 'UglifyJS']
...
for i in range(len(to_do_list)):
    try:
        ...
        # print(touple_list)
        output_file = open(APPLICATION_NAME+"_missing_edge_log_"+MODEL_NAME+".txt","w+")
        cutoff_thrshld = 0.9
        node_df_org = pd.read_csv("../prune_new/"+APPLICATION_NAME+"_node.csv")
